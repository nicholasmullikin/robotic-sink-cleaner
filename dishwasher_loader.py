...
def load_arm():
    ...
    panda_id = p.loadURDF("franka_panda/panda.urdf", pandaStartPos, panda_start_orient, globalScaling=1.5,
                          useFixedBase=1)
    return panda_id
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        if t < 1:
            ...
            if diff < 0.075 and totalCounter > 40:
                if not point_cloud_calc:
                    pointCloud = get_point_cloud(img, near, far, fov, pandaId)
                    plot_point_cloud(pointCloud)
                    # to_ply_file(pointCloud)
                    point_cloud_calc = True
